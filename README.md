...
除上述多视图几何方法外，还可通过单目深度估计模型从单帧图像直接获取 3D 信息：

1. **模型**：使用 YOLO-depth 模型（`yolo26s-depth.pt`），在 YOLO 检测框架基础上增加深度估计头，单次推理同时输出目标检测框和逐像素深度图（单位：米）。
2. **3D 反投影**：给定像素 `(u, v)` 及其深度 `d`，结合相机内参 `K`，通过 `X = d · K⁻¹ · [u, v, 1]ᵀ` 恢复该像素对应的 3D 坐标。
3. **优势**：单帧即可获取稠密 3D 信息，无需多帧匹配或特征点对应，对纹理稀疏区域更鲁棒。
4. **局限**：深度精度依赖模型泛化能力，绝对尺度可能漂移；无法直接恢复相机位姿，需配合 PnP 或 ICP 等方法。

脚本：`scripts/Q3_1/Depth_model_estimation.py`、`scripts/Q3_1/example_depth_model.py`
